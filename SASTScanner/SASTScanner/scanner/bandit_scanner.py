...
def _severity_map(bandit_severity):
    return {
        ...
        'MEDIUM': 'MEDIUM',
        'LOW':    'LOW',
    }.get(bandit_severity.upper(), 'INFO')
...
def run_bandit(source_dir):
    ...
    result = subprocess.run(
        [bandit_bin, '-r', source_dir, '-f', 'json', '-ll', '--quiet'],
        capture_output=True,
        text=True,
        timeout=300,
    )
    ...
    try:
        ...
        raise RuntimeError(
            f"Bandit returned non-JSON output. "
            f"Binary: {bandit_bin}. "
    ...
    for issue in data.get('results', []):
        ...
        findings.append({
            ...
            'recommendation': BANDIT_RECOMMENDATIONS.get(
                test_id, 'Review the code and apply security best practices.'
            ),
            'tool':           'Bandit',
